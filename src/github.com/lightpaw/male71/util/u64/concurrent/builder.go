...
func NewBufferCacheMapBuilder(f func(key uint64) (pbutil.Buffer, error)) *BufferCacheMapBuilder {
	return NewVersionBufferCacheMapBuilder(func(key uint64, version uint64) (pbutil.Buffer, error) {
		return f(key)
	})
}
...
type Func func(key uint64, version uint64) (pbutil.Buffer, error)
...
func (b *BufferCacheMapBuilder) Build() U64BufferMap {
	if b.expireAfter > 0 {
		// 有过期时间
		return newVersionBufferCacheMap(b.f, b.timeProvider, b.expireAfter)
	}

	return concurrent.NewU64VersionBufferCacheMap(b.f)
}
