	...
	for chunkSize := minSize; chunkSize <= maxSize; chunkSize *= factor {
		...
		pool.classes[n].New = func(size int) func() interface{} {
			return func() interface{} {
				buf := &RecycleBuffer{buf: make([]byte, size)}
				return buf
			}
		}(chunkSize)
		n++
...
func (pool *SyncPool) Alloc(size int) *RecycleBuffer {
	if size <= pool.maxSize {
		for i := 0; i < len(pool.classesSize); i++ {
			if pool.classesSize[i] >= size {
				mem := pool.classes[i].Get().(*RecycleBuffer)
				mem.buf = mem.buf[:size]
				mem.freed = false
				return mem
			}
		}
	}
	return &RecycleBuffer{buf: make([]byte, size)}
}

// Free release a []byte that alloc from Pool.Alloc.
func (pool *SyncPool) Free(mem *RecycleBuffer) {
	if size := cap(mem.buf); size <= pool.maxSize {
		for i := 0; i < len(pool.classesSize); i++ {
			if pool.classesSize[i] >= size {
